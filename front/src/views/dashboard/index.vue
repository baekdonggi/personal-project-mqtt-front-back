  ...
  data() {
    return {
      selected: {
        ...
        tagList: ['humidity', 'temperature', 'test'] // TODO: 현재 화면에서 출력할 태그 이름(deviceId선택 시 해당 장비의 태그를 설정할 수 있도록 한다.),
  ...
  methods: {
    createMqtt() {
      ...
      mqttClient.on('connect', () => {
        // mqtt연결 시 구독한다.
        const topic = 'tempsensor' // 구독할 topic
    ...
    makeChartLabels(mqttData) {
      ...
      this.chartLabels.push(time) // datetime을 사용한다.(분:초만 추출함)
    },
    makeDatasetDatas() {
      // 데이터셋의 데이터 추출
      const datasetDatas = []
      ...
      for (let i = 0; i < this.chartDatasetLabels.length; i += 1) {
        ...
        for (let j = 0; j < this.mqttDataList.length; j += 1) {
          ...
          // console.log(`mqttData[${j}]: ${mqttData}`)
